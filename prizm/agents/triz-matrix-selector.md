...
You **update `${RUN_DIR}/02_selection.json` in place** (read it first, modify the relevant fields, write it back). The schema is `prizm/schemas/02_selection.schema.json`. You are responsible for keeping the artifact valid.
