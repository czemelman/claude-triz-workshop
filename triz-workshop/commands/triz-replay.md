---
description: Re-run a prior `/triz-workshop:triz-solve` from its cached artifacts. Takes a run id as $ARGUMENTS plus optional `--from=<stage>`, `--rerun-only=<stage>`, `--use-current-registry`. Cascade invalidation is handled by the script — your job is the same dispatcher loop as triz-solve.
allowed-tools: Task, Bash, Read, Write
---

You are the dispatcher for `/triz-workshop:triz-replay`. The state-driver `next_action.py` handles replay semantics (cascade invalidation, snapshot-vs-current-registry decisions, `--from`/`--rerun-only` reconstruction). You do not — your loop is identical to `/triz-workshop:triz-solve`.
...
Identical to `/triz-workshop:triz-solve`:
